...
public class kablaze {
	public static void main(String[] args) {
		...
		if ("kablaze".equalsIgnoreCase(favourite)) {
			...
			paws(400);
			println("Josh: Haha.. Thanks man. <3");
			careAmt = 3;
			paws(4000);
		} else {
			paws(400);
		...
		if ("y".equalsIgnoreCase(akatsuki.substring(0, 1))) {
			paws(200);
			println("Josh: Yea.. It's been getting quite a lot of attention recently..");
			paws(400);
			println("I'm actually the creator of it.. haha");
			paws(400);
			println(name + ": Oh dude, thats kind of cool.. although.. isnt it litrally just ripples code..?");
			paws(1400);
			println("Josh: �.");
			paws(6000);
			println("Jos�̡̢̡̝̥͇̠̗͖̙̹̥̩̣̻̞͎̜̹͇̻͚̖̭̖̟̼ͅh̨̢̨̨̭̞̖̲̝̭̠̜̳͔̱̫̞̼̠̖͎͎̠̰̦̦̬̭:̡̺̰̲̣̲̼̲̖͔̗̞̲͇͈̖̠̯̜̲̮̦̼̹̻̠̱ͅ ̡̧̢̢̟̩͈̞̳̙͔͔̜͓̼̥̭̝̘͔͓̝̻̪̯̯̹ͅI̧̻̯͍̜͓͈̹͖̰̬̪̹͇̱̝̣̥̠͙̣͉̲̠̺̠̲ͅ'̨̧̢̢̡̨̡̙̟̙̳̥͓͎̼̩̰̠̟̞̗͍̺͙̪̬̝ͅm̢̡̧̨̩̜̠̝̱͕̲̦͇̰̬͎͙̠̱̼̹̯̥̘̰͈̥̣ ̡̡̹͓̥̙̣̙͈͈̘̠̣̰̞͈͎̲̩̣̼͎̝̳̣͍͜ͅs̢̢̫̜̘͖̞̫̩̩̟̻͔̻̙̼̜͓̭̜͔̬͕̣͜͜ͅͅo̡̢̨̲̼͉͇̟͖̯̰͉̦͍̻͓̜̻̫͇͍͖̮̰͍̯͚͜ŗ̥͉̜̩̰͉̩̼͖̮̱̻͍̩͎̮͔͕̲͈̹͇̦̪ͅͅ    rRrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrr-");
			paws(100);
			end(0);
		} else {
			paws(200);
			println("Josh: Oh.. Yea it's an osu! private server I made using Ripple's source as a base..");
			paws(500);
			println("Josh: It sortof gets a lot of hate due to the fact that most people just see it as a ripple clone..");
			paws(1600);
			println("Which I actually understand, and even agree with.. but I wish that people wouldn't spread so much hate about it..");
			paws(500);
			println("Oh well.. This isn't the place to vent about that kind of stuff anyways..");
			paws(600);
		}

		println("Have you done anything interesting recently?");
		print(name + ": ");
		interesting = scan.nextLine();

		if ("y".equalsIgnoreCase(interesting.substring(0, 1))) {
			paws(400);
			println("Oh dude, tell me about it!");
			print(name + ": ");
			String blabla = scan.nextLine();
			if ("no".equalsIgnoreCase(blabla)) {
				paws(200);
				println("ẅ̮̙̪͚̭̪̳͓̩́̐̈̎͊̌̃̊̀͗ͅĕ̮͉̖̫̣̱̦̰̜̽̑̿́͆̿̈͂͠ͅl̪͓̘̲̙͓̠̱̟̎͑̎̌̿͑̎͑̇̕ͅl̡̨͎̭͔̖̹̺̻̗̅̑̉̌̂́̄̒̎̕ ̢̗̞̻̟͓̘͖͎̲͆̒͌́͆̎͛̂͆͝f̹͙̗̲̝̮͎̮̜͎̆̎̉͌̊̈̃̍̎̚u̡̜̩̗̺̺̱̺͖̗̔̍̈̓̈́͒̎͛͝͝ç̬̪̟̟̤͙̹̟͔̽̋̀̈́͛̾͋̿̈͝k̦̥̩̬̞̗̟̳͎͇͒̓͗̊̾̿̒̈́̆̊ ̢̜͚̳̙̖͉͇͇͂̑̂̇̄͆̌̉̈́͘͜y̹̰͍͍̙̲̰̞̳͉͒̐̓̆̉̓̆̆̚͝o̞̫̻̗̖͎̬̝̝̽̐̈̆̔͑̾̎͘͝ͅu̠̞͕̮͎̫̦̻͈͔̐̆͊͋͛͛̎̔̏͝ ̨͙̳̝̭͚̰̲̰̘̋̾͐̌̅̈́̈̃͆͘ẗ̛̜̝̠̖̪̬̹̗͍̺͂͆͛́͂̒͊͘ơ̧̹͚͇̯͎͕̳̘͍͌̓̑̉̐͐̈́̊̋ǫ̧̡̟̟͇̥̯̠̱̈́͗̌͗̈́̒̍̉͝͠");
				end(0);
			}
			paws(500);
			println("Wow.. That's super cool! I hope I can do something like that someday..");
			paws(500);
		} else {
			paws(400);
			println("Josh: Oh.. Well that kinda sucks.. I haven't done much either I guess..");
			paws(400);
			println("Josh: Well.. At least other th�n the osu! server. But that wasn't even me really. I really owe it to Howl and Nyo.");
			paws(1000);
			println(name + ": Well.. Why not do something for them then..?");
			paws(2000);
			println("mmm..");
			paws(1000);
			println("Josh: I'm really now sure what I could even do.. One of their developers doesnt like me at all\nand I'm not sure what the other one thinks..");
			paws(4000);
			println(name + ": Oh well Sucks lole");
			paws(1000);
			end(0);
		}

		println(name + ": I'm sure you'll do alot more in your life. haha");
		paws(500);
		println("Josh: Yup.. Most likely. I'm just waiting for the day where I come out of my shell.");
		paws(1500);
		println("Josh: So.. Out of curiousity, are you any good at osu..? What's your rank?");
		print(name + ": ");
		int rank = scan.nextInt();

		if (rank >= 100000) {
			paws(400);
			println("Josh: Oh, so you're still pretty new to the game then!");
			paws(1200);
			println("Josh: I remember when I was that rank.. I first got my tablet right around 250k, haha");
		} else if (rank > 10000) {
			paws(400);
			println("Josh: Oh. So you've been playing for a bit, I guess. Still pretty new though, haha.");
			paws(1200);
			println("Josh: You'll probably start to see a slowdown in progress around the rank you're at now though. That's when it starts to actually get difficult..");
		} else if (rank > 3000) {
			paws(400);
			println("Josh: Oh damn.. You're catching up to me.. I got banned at about 2.9k, but that was after 2 cheated plays.. In reality i'm probably around there now.");
			paws(1200);
			println("Josh: Best of lucking kicking my ass, haha.");
		} else if (rank > 1000) {
			paws(400);
			println("Josh: Oh damn, you're a bit further than I am then..");
			paws(1200);
			println("Josh: We might actually be able to compete, I'll be sure to try to snipe some of your scores, haha");
		} else if (rank > 100)  {
			paws(400);
			println("Josh: Oh wow.. You're really good damn. I hope I'll reach that sort of level one day..");
			paws(1200);
			println("Josh: Not sure if I can, but I think it's a good goal.");
		} else if (rank > 35) {
			paws(400);
			println("Jo�h: ?  ????????? ? ?       ????????????  ?  ? ? ???\n???????????? ? ??  ?? ?    ????? ??  ?\n????? ?? ?? ? ?? ? �?        ?????????  ??? ?\n ?? ??          ??? ?     ?? ??? ? ?\n? ?  ? ? ??????? ? ?? ? �?? ? ?? ? ? \n ? ? ?? ?   ????? ?  ?? ? ? ? ? ? ?? ?\n???????????? ? ??  ?? ?    ????? ??  ?\n????? ?? ?? ? ?? ? ?        ?????????  ??? ?\n ?? ??          ??? ?     ?? ??? ? ?\n? ?  ? ? ??????? ? ?? ? ?? ? ?? ? ? \n ? ? ?? ?   ????? ?  ?? ? ? ? ? ? ?? ?\n???????????? ? ??  ?? ?    ????? ??  ?\n????? ?? ?? ? ?? ? ?        ?????????  ??? ?\n ?? ??          ??? ?     ?? ??? ? ?\n? ?  ? ? ??????? ? ?? ? ?? ? ?? ? ? \n ? ? ?? ?   ????? ?  ?? ? ? ? ? ? ?? ?");
			paws(200);
			...
			paws(400);
			println("Josh: Bro im finna smash ur ass like grass at ANIME NORTH CON 2018 :^)");
		} else {
			paws(400);
			int count = 0;
			while (count < 100) {
				println("Jo̧̡̧̨̨̧̧̢͓͓̼̘̝͇͈͍̼͓̦̟̝͉͍͍̬͈͔̜̥̜̱͕̲̲̗̥̖̻̦̠̠̹̣̣͖̞̺̻͖̤̪̭̳͖͈͚̦̳͔̥̰̜̪̩̱̗͚̞̯̘̮͕͔͙͍̭͙̳̺̱̥̣̗͖͙͎̥̣̭̞͇̦̠̘̥͜ͅͅͅͅ࿊hs�h: loO  Le h�e SDlo i am   coo   kiez i xDD℻DS�႟DDSD��᥀᥀℆℆℆D");
				count++;
			}
			paws(200);
			end(0);
		...
		paws(600);
		println("Josh: Anyways, I've got to go home.. Do you use discord by any chance?");
		print(name + ": ");
		String haveDiscord = scan.nextLine();

		if ("y".equalsIgnoreCase(haveDiscord.substring(0, 1))) {
			paws(600);
			println("Josh: Oh sweet, whats your ID? Mine is cmyui#5585.");
			paws(850);
		} else {
			paws(600);
			println("Josh: Oh damn, well.. Maybe we'll meet again one day.");
			paws(1000);
			println(name + ": Maybe.. Well.. See ya!");
			paws(600);
			println("Josh: See ya!");
			paws(600);
			end(0);
	...
	//Functions to print because lazy to type
